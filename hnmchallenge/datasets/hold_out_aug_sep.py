...
class AugSepDataset(DatasetInterface):

    DATASET_NAME = "AugSep_dataset"
    ...
    def create_holdin_holdout(self) -> None:
        ...
        # create holdin
        cond = fd["t_dat"].isin(hold_out["t_dat"])
        fd.drop(fd[cond].index, inplace=True)
        hold_in = fd
