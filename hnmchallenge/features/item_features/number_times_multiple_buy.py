...
class NumberBought(ItemFeature):
    ...
    def _create_feature(self) -> pd.DataFrame:
        ...
        number_bought = data_df.groupby([DEFAULT_USER_COL, DEFAULT_ITEM_COL])["price"].count().rename("number_bought").reset_index().drop([DEFAULT_USER_COL], axis=1)
        count=[]
        ...
        feature = number_bought.groupby([DEFAULT_ITEM_COL])["count"].count().rename("count").reset_index()
...
if __name__ == "__main__":
    ...
    for kind in ["full", "train"]:
        feature = NumberBought(dataset, kind)
