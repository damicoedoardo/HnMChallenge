...
class PopSales2Cumulative(ItemFeature):
    FEATURE_NAME = "popularity_sales_channel_2_cumulative"
    ...
    def _create_feature(self) -> pd.DataFrame:
        ...
        for idx, i in enumerate(intervals):
            ...
            data_df = fd.loc[m]
            duplicated_rows = data_df.drop_duplicates(
                [DEFAULT_USER_COL, DEFAULT_ITEM_COL]
            )
            duplicated_rows = duplicated_rows[duplicated_rows["sales_channel_id"] == 2]
            count_mb = duplicated_rows.groupby(DEFAULT_ITEM_COL).count()
            feature = count_mb.reset_index()[[DEFAULT_ITEM_COL, "t_dat"]].rename(
                columns={"t_dat": f"popularity_{idx+1}_sales_channel_2_cumulative"}
            )
            feature[f"popularity_{idx+1}_sales_channel_2_cumulative"] = (
                feature[f"popularity_{idx+1}_sales_channel_2_cumulative"]
                - feature[f"popularity_{idx+1}_sales_channel_2_cumulative"].min()
            ) / (
                feature[f"popularity_{idx+1}_sales_channel_2_cumulative"].max()
                - feature[f"popularity_{idx+1}_sales_channel_2_cumulative"].min()
            )
