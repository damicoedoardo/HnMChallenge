...
class PopSales1Cumulative(ItemFeature):
    FEATURE_NAME = "popularity_sales_channel_1_cumulative"
    ...
    def _create_feature(self) -> pd.DataFrame:
        ...
        for idx, i in enumerate(intervals):
            ...
            feature = count_mb.reset_index()[[DEFAULT_ITEM_COL, "t_dat"]].rename(
                columns={"t_dat": f"popularity_{idx+1}_sales_channel_1_cumulative"}
            )
            feature[f"popularity_{idx+1}_sales_channel_1_cumulative"] = (
                feature[f"popularity_{idx+1}_sales_channel_1_cumulative"]
                - feature[f"popularity_{idx+1}_sales_channel_1_cumulative"].min()
            ) / (
                feature[f"popularity_{idx+1}_sales_channel_1_cumulative"].max()
                - feature[f"popularity_{idx+1}_sales_channel_1_cumulative"].min()
            )
